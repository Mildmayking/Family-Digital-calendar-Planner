...
function App() {
  ...
  if (loading || !refs.auth || !refs.db) return <div className="h-screen flex items-center justify-center"><lucide.Loader className="animate-spin text-blue-500"/></div>;
  
  if (!user || view === 'auth') return <AuthScreen onAuth={handleAuth} onGuestLogin={handleGuestLogin} />;
  
  if (view === 'license_gate') return <LicenseGate verifyLicense={verifyLicense} signOut={()=>refs.auth.signOut()} />;

  if (view === 'setup_family') return <FamilySetupScreen onSetup={handleFamilySetup} signOut={()=>refs.auth.signOut()} />;

  if (view === 'profiles') return <ProfileSelector members={members} onSelect={m=>{setCurrentMember(m); setView('home');}} onCreate={createMember} signOut={()=>refs.auth.signOut()}/>;
