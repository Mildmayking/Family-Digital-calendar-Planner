...
<head>
    ...
    <!-- React and Babel for JSX transpilation (Needed for components) -->
    <script src="https://unpkg.com/react@18/umd/react.development.js"></script>
    <script src="https://unpkg.com/react-dom@18/umd/react-dom.development.js"></script>
    <script src="https://unpkg.com/@babel/standalone/babel.min.js"></script>

    <!-- DIRECT FIREBASE SDK LOADS (Final, Stable CDN) -->
...
<body>
    ...
    <script type="text/babel">
        // ACCESS GLOBAL OBJECTS AFTER SDK SCRIPTS HAVE RUN
        const { useState, useEffect, useRef } = React;
        const lucide = window.lucide; 

        // --- FIREBASE CONFIGURATION (Managed Service Model) ---
        // KEYS ARE HARDCODED HERE FOR RELIABILITY
        ...
        function initFirebase() {
            // We check for the global 'firebase' object to ensure SDKs loaded first
            if (!appInstance && typeof firebase !== 'undefined' && firebase.auth && firebase.firestore) {
        ...
        function App() {
          ...
          useEffect(() => {
            ...
            const membersQuery = refs.db.collection('artifacts').doc(appId).collection('public').doc('data').collection(COLLECTIONS.MEMBERS).where('familyId', '==', familyId);
          ...
          useEffect(() => {
            ...
            const q = refs.db.collection('artifacts').doc(appId).collection('public').doc('data').collection(COLLECTIONS.EVENTS).where('familyId', '==', familyId);
          ...
          const handleAuth = async (e, email, password, isLogin) => { 
              ...
              try { 
                  if (isLogin) {
                      await refs.auth.signInWithEmailAndPassword(email, password); 
                  } else {
                      await refs.auth.createUserWithEmailAndPassword(email, password);
                  }
              } catch (e) { 
                  throw e;
              } 
          ...
          const verifyLicense = async (licenseKey) => {
              ...
              const licenseRef = refs.db.collection(COLLECTIONS.LICENSES).doc(licenseKey);
              ...
              try {
                  ...
                  await refs.db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('settings').doc('config').set({ 
          ...
          const handleFamilySetup = async (mode, code) => {
              ...
              const familyMembersQuery = refs.db.collection('artifacts').doc(appId).collection('public').doc('data').collection(COLLECTIONS.MEMBERS).where('familyId', '==', fid).get();
              ...
              await refs.db.collection('artifacts').doc(appId).collection('users').doc(user.uid).collection('settings').doc('config').set({ familyId: fid, joinedAt: Date.now() }, { merge: true });
              setFamilyId(fid);
              
              const isOwner = mode === 'create';
              
              if (isOwner) {
                await refs.db.collection('artifacts').doc(appId).collection('public').doc('data').collection(COLLECTIONS.MEMBERS).add({ 
                    familyId: fid,
                  ...
                  const userInFamilyQuery = refs.db.collection('artifacts').doc(appId).collection('public').doc('data').collection(COLLECTIONS.MEMBERS).where('familyId', '==', fid).where('createdBy', '==', user.uid).get();
          ...
          const createMember = async (name, role, gender, theme, avatar) => {
              if (members.length >= 6 || !refs.db) return; 
              await refs.db.collection('artifacts').doc(appId).collection('public').doc('data').collection(COLLECTIONS.MEMBERS).add({ 
          ...
          if (loading || !refs.auth || !refs.db) return e('div', { className: "h-screen flex items-center justify-center" }, e(lucide.Loader, { className: "animate-spin text-blue-500" }));
          
          if (!user || view === 'auth') return e(AuthScreen, { onAuth: handleAuth, onGuestLogin: handleGuestLogin });
          
          if (view === 'license_gate') return e(LicenseGate, { verifyLicense: verifyLicense, signOut: () => refs.auth.signOut(), auth: refs.auth });

          if (view === 'setup_family') return e(FamilySetupScreen, { onSetup: handleFamilySetup, signOut: () => refs.auth.signOut() });

          if (view === 'profiles') return e(ProfileSelector, { members: members, onSelect: m => { setCurrentMember(m); setView('home'); }, onCreate: createMember, signOut: () => refs.auth.signOut() });

          const theme = THEMES[currentMember?.theme || 'ocean'];

          const Header = e('header', { className: `px-4 py-3 sticky top-0 z-30 flex justify-between items-center backdrop-blur-xl bg-opacity-90 border-b border-black/5 ${theme.bg}` },
            e('div', { className: "flex items-center gap-3", onClick: () => setView('profiles') },
                e('div', { className: `h-10 w-10 rounded-full flex items-center justify-center text-xl shadow-md border-2 border-white/40 ${theme.accent} text-white` }, currentMember.avatar),
                e('div', null, e('h1', { className: "text-base font-bold leading-tight" }, currentMember.name))
            ),
            e('button', { onClick: toggleMusic, className: `p-2 rounded-full ${theme.card} border border-black/5` }, 
                isPlayingMusic ? e(lucide.Volume2, { size: 20, className: theme.accent.replace('bg-', 'text-') }) : e(lucide.VolumeX, { size: 20, className: "opacity-40" })
            )
          );

          const MainContent = e('main', { className: "p-4 pb-32 max-w-xl mx-auto w-full min-h-[85vh]" },
            view === 'home' && e(Dashboard, { member: currentMember, events: events, theme: theme, setView: setView, db: refs.db, appId: appId }),
            view === 'notes' && e(NotesManager, { member: currentMember, familyId: familyId, db: refs.db, appId: appId, theme: theme }),
            view === 'planner' && e(FamilyCalendar, { member: currentMember, members: members, events: events, familyId: familyId, db: refs.db, appId: appId, theme: theme }),
            view === 'settings' && e(SettingsScreen, { member: currentMember, members: members, familyId: familyId, auth: refs.auth, theme: theme, onUpdate: setCurrentMember })
          );

          const Navbar = e('nav', { className: "fixed bottom-0 left-0 right-0 z-40 p-4" },
            e('div', { className: `max-w-xl mx-auto ${theme.card} border rounded-2xl p-2 flex justify-between shadow-2xl backdrop-blur-xl` },
                [{id:'home',icon:lucide.Home}, {id:'notes',icon:lucide.PenTool}, {id:'planner',icon:lucide.CalendarIcon}, {id:'settings',icon:lucide.Settings}].map(i => e('button', { 
                    key: i.id, 
                    onClick: () => setView(i.id), 
                    className: `flex-1 flex flex-col items-center py-3 rounded-xl transition-all ${view === i.id ? '' : 'opacity-60'}` 
                },
                    e('div', { className: `p-1.5 rounded-lg ${view === i.id ? `${theme.accent} text-white shadow-lg -translate-y-1` : 'text-current'}` }, 
                        e(i.icon, { size: 24, strokeWidth: view === i.id ? 2.5 : 2 })
                    )
                ))
            )
          );

          return e('div', { className: `min-h-screen transition-colors duration-500 ${theme.bg} ${theme.text}`, style: { fontFamily: currentMember?.role === 'child' ? '"Comic Neue",cursive' : '"Inter",sans-serif' } },
            e('audio', { ref: audioRef, loop: true, src: "https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3?filename=lofi-study-112762.mp3" }),
            Header,
            MainContent,
            Navbar
          );
